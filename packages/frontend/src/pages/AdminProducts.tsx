            ...
            {products.map(p => (
              <tr key={p.id} data-testid={`admin-product-row-${p.id}`} className="border-b border-gray-100 dark:border-gray-700 text-gray-700 dark:text-gray-300">
                ...
                <td className="flex gap-2 py-3">
                  <button data-testid={`admin-product-edit-desktop-${p.id}`} onClick={() => { setEditing(p); setForm({ name: p.name, description: p.description ?? '', price: p.price, stock: p.stock, imageUrl: p.imageUrl ?? '' }); }} className="bg-yellow-400 text-gray-900 px-3 py-1 rounded-lg text-xs font-semibold">Edit</button>
